    ...
    execute(args, message) {
        if(!args[0]) {
            ...
            WEATHER.find({search: position, degreeType: 'C'}, (err, result) => {
                if(err) {
                    EMBED.reply("Oups, il semblerait qu'il n'y a pas de données pour cette position !", message, 'RED');
                } else {
                    result = result[0];
                    if(result == undefined) {
                        EMBED.reply("Oups, il semblerait qu'il n'y a pas de données pour cette position !", message, 'RED');
                        ...
                        EMBED.reply(`**Données météo pour __${result.location.name} ${coords == true ? " (" + parseFloat(result.location.lat).toFixed(2) + ", " + parseFloat(result.location.long).toFixed(2) + ")" : ""}__**\n\n**Température** : ${result.current.temperature}°C (ressenti: ${result.current.feelslike}°C)\n**Ciel** : ${skytext}\n**Humidité** : ${result.current.humidity}%\n**Vent** : ${result.current.windspeed}\n\n**Observation à** : ${result.current.observationtime + " (UTC"+result.location.timezone+")"}`, message, 'GREEN');
                    }
                }
            });
        }
