...
const COMPLETER = function(line) {
    let completions = BOT.COMMAND_MANAGER.all(true).map(command => command.getName());
...
function start(){
    ...
    READER.on("line", async (input) => {
        ...
        let command = BOT.COMMAND_MANAGER.get(commandName, true);
    
        if(command){
            let execute = await command.execute(args);
            if(execute == false){
                BOT.LOGGER.cli("Invalid usage, try : " + command.getUsage());
            }
        } else if(commandName.length > 0) {
            BOT.LOGGER.cli("Undefined command, try 'help' or TAB to get the command list");
        }
